    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        max_belief = [max(distance, key=lambda x:distance[x]) for distance in livingGhostPositionDistributions]
        min_value = float('inf')
        min_action = None

        for action in legal:
            value = min(self.distancer.getDistance(Actions.getSuccessor(pacmanPosition,action), ghost) for ghost in max_belief)
            if min_value > value:
                min_action = action
                min_value = value
